...
export const uploadImagesViaForm = [
  uploadFolderArray,
  handleMulterError,
  async (req, res) => {
    try {
      ...
      // 👇 TRẢ VỀ HTML (cho iframe có thể parse)
      const responseData = {
        success: true,
      ...
      const htmlResponse = `
        ...
        <body>
          ...
          <div style="display:none;">Upload Complete</div>
        </body>
        </html>
      `;
      ...
      console.error('❌ Error in uploadImagesViaForm:', error);
      
      const errorHtml = `
        <!DOCTYPE html>
        <html>
        <body>
          <script type="application/json" id="upload-error">
            ${JSON.stringify({
              success: false,
